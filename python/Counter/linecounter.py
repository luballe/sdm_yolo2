...
class selectLine:
    ...
    def __init__(self,imag,ownString='Seleccione un par de puntos para crear una linea ',filename='Linea_de_conteo.jpg',linecount=1,DefaultPoints=False,pt1=(-1,-1),pt2=(-1,-1),sentido='SENTIDO'):
        ...
        if DefaultPoints:
            ...
            while(1):
                ...
                if k == 27 or k==ord('q') or k==ord('Q') or k==ord('s') or k==ord('S'):
                    ...
                    break
            filenameraw, file_extension = os.path.splitext(filename)
            path_to_file, file_basename = os.path.split(filenameraw)
            #self.archivosalidajpg=filenameraw+'_linea_'+str(linecount)+'.jpg'
            #self.archivosalidajpg=filenameraw+'_linea_'+str(linecount)+'.jpg'
            #self.archivosalidajpg=path_to_file+os.sep+sentido+'_'+file_basename+'.jpg'
            self.archivosalidajpg=path_to_file+os.sep+sentido+'.jpg'
            cv2.imwrite(self.archivosalidajpg,img)
            ix,iy=-1,-1
            uux,iiy=-1,-1
            ox,oy=-1,-1
...
#            self.FILE.write(str(num2clases[jj])+';'+str(self.counterclases[jj])+';'+str(float(frame)/float(self.FPS))+'\n')
        line=""
        for jj in range(len(self.counterclases)):
            #print(type(self.counterclases[jj]))
            line=line+str( self.counterclases[jj].astype(np.int64)  )+','
            #line=line[:-1]
...
class counter:
    ...
    def __init__(self, pt1,pt2,filename="salidaconteo.csv",fps=20,linecount=1,sentido="SENTIDO"):
        #print("pt1:")
        #print(pt1)
        #print("pt2:")
        #print(pt2)
        ...
        self.filename_output=path_to_file+os.sep+sentido+'_'+file_basename+'.csv'
        #print("Filename")
        #print(self.filename_output)
        #self.filename_output_line=filenameraw+'_linea_'+str(linecount)+'.lin'
        self.filename_output_line=path_to_file+os.sep+sentido+'_'+file_basename+'.lin'
