...
def FFMPEG_Merge_Video_Files(path, sources_list):
    ...
    cmd = '/usr/bin/ffmpeg -f concat -safe 0 -i "{}" -c copy "{}"'.format(os.path.join(results_path,file_name),os.path.join(results_path,result_name))
    command=cmd.split(" ")
    # print some process information
    ...
    #p = subprocess.Popen(cmd, shell=True, bufsize=0, stdout=subprocess.PIPE, universal_newlines=True)
#    cmd_out=cmd
...
#8******************************************************************************************************************************OK
def rename_split_results(results_path, Camera_Name, ini_time_str, YYYYMTDD):
    ...
    dirlist.sort()
    duration_mins=int(MaxDuration/60)
    for image_file in dirlist:

        #print("video__Start_Time: "+ini_time_str)
        ...
        if image_file.endswith("avi"): 
            ...
            end_hour      = ini_time_str[0:2]
            end_mins      = ini_time_str[2:4]
            tmp_end_str   = end_hour+":"+end_mins
            date_format_str = '%H:%M'
            tmp_end_time = datetime.datetime.strptime(tmp_end_str, date_format_str)
            end_time = tmp_end_time + datetime.timedelta(minutes=duration_mins)
            end_time_str = end_time.strftime('%H%M')
           
            #print("ini_time: "+ini_time_str)
            ...
            new_name = '{}'.format(ini_time_str) + '-' '{}.avi'.format(end_time_str)
            ...
            ini_time_str = end_time_str
       #         Start_HHMM      = time.strftime('%H:%M', time.gmtime(int(MM)*60+int(HH)*3600))
            #video__Start_Time = time.strftime('%H:%M:%S', time.gmtime(video__Start_Time)).replace(':','')
            #video__Start_Time = time.strftime('%H:%M', time.gmtime(video__Start_Time)).replace(':','')
            #print("video__Start_Time_2: "+video__Start_Time)
        ...
        if image_file.endswith("avi"): 
            ...
            if len(partial_info) ==6:
                ...
                directory_list.append({ 'File_Name'      :image_file,                               # File Name
                                        ...
                                        'Start_Time_HHMM':Start_HHMM,                               # Video Start Time in format HH:MM
...
if __name__ == '__main__':
    ...
    path = sys.argv[1]
    print("**** Starting fragmentation of: "+ path)
    ...
    rename_split_results(path_results, 
                         ...
                         sources_directory_list[0]['Start_Time_HHMM'].replace(':',''), 
